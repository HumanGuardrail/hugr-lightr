...
HuGR (the company / brand)
 └─ CoreLink (the platform)
     ...
     └─ Cell        — runtime front door, local-first          (THIS REPO)
...
- Cell is **design-phase**: no benchmark, install count, or perf number may
...
- **`Engine` lineage from `corelink-runners`** (`corelink-runner/src/
  ...
  lifecycle. In the cloud, Cell is what a runner lease executes; Runners is
  the fabric, Cell is the runtime.
...
repos is not.** Only work on hugr-cell here.
...
- Crate name `hugr-cell`, binary `cell` (crates.io `cell` is taken).

## Don't touch

- Other HuGR projects share the parent dir — never mutate siblings.
- `docs/whitepaper/hugr-cell-v1.md` §9 principles and the tense-discipline
