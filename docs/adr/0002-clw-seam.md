...
independently-evolving repos with no code dependency allowed**. Cell and clw
...
2. The sibling repo is **never mutated** from this repo (standing fence). If
   Cell needs a clw change, it is requested via the owner/clw session.
...
4. `publish = false` on every cell crate until the distribution decision
