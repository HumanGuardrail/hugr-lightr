...
One line: `cell-store::LocalStore` implements clw's `CasTransport` +
...
1. **Layout** (sharded like clw-cache, two top-level namespaces):
   ...
   ```
   Default root `~/.cell/store`; override via `CELL_STORE_DIR`.
...
7. **Accepted cost (documented, not debt):** clw pipelines also maintain the
   L1 `LocalCache` (`~/.cell/cache`, override `CELL_CACHE_DIR`), so touched
