# HuGR Cell — Product Whitepaper (v1)
...
is heavy on a laptop and expensive in a fleet. HuGR Cell unbundles them: a
...
commodity. The substrate that makes Cell economically different — instant
...
| Docker bundles | Cell's answer | Status |
|---|---|---|
...
HuGR (the company / brand)
 └─ CoreLink (the platform)
     ...
     └─ Cell        — the runtime front door, local-first      (THIS REPO)
   hugit (the forge for agent fleets)                          (campaign #3)
```

Cell is the local-first front door of the same fabric Runners sells: locally
it runs standalone and free; in the cloud, Cell is what a runner lease
executes. Distribution is the clw pipeline; the registry is CoreLink's CAS/AC
over HTTPS, tenant-namespaced, PAT-authed. Cell is a **pure client** — zero
...
## 6. The funnel (the business reason Cell exists)
...
Workspaces pitches write themselves. Cell carries no bill of its own; it is
...
built closed versions of this machinery for internal use; Cell is that
...
3. **Free local, forever, no account.** Stage 1 touches no servers. The
   funnel dies the day the first `cell run` needs a login.
...
- **v0.1 — feel the thesis (local).** `cell run` native engine, clw pipeline,
...
- **v1.x — the cloud half.** `fc` engine, lazy rootfs, snapshot pool;
  Cell as the runtime a Runners lease executes.

## 11. Risks & non-goals

- **OrbStack comparison on day one** — if Cell local is not spectacular
...
Two-thirds of Cell predates Cell, in production or shipped: the cache with
