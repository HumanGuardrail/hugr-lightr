...
| F-205 | vz engine boot-never | 🟡 | **Intel boot bring-up (2026-06-12):** `--features vz` compiles+links+**RUNS** here (`engine ls` honest, codesigned w/ entitlement, swift rpath baked); real x86_64 kernel built (Linux 6.18.5; virtio/vsock/virtiofs =y) + `lightr-init` (musl) + pack assembled→installed → **`vz available`**. **Blocker (arch-independent, NOT Intel-specific):** the exit-code channel uses raw `AF_VSOCK`, which macOS has NO host support for — `socket(AF_VSOCK)`=ENODEV on Intel AND ARM, so bind fails pre-boot. Needs the Swift-shim `VZVirtioSocketDevice` OR an exit-file on the (writable) virtiofs rootfs (decisions-log 2026-06-12). 4 latent bugs fixed en route: pack_dir path, swift rpath, kernel sha256 pin, entitlement XML |
| F-206 | Apple kernel + Rust PID1 | 🟡 | kernel-pack pipeline validated **end-to-end on Intel** (build→assemble→install→probe = `vz available`); `lightr-init` musl guest built + packed; `verify_pack` wired into `install-pack`. The exit-code DELIVERY (F-206 core) is blocked on the macOS vsock rework above — `lightr-init`'s AF_VSOCK write lands only on a host that receives it (Linux `fc`), or via a macOS-native channel. On ANY Mac the rework is the same (`spikes/s5-vz-boot-arm64/` for arm64) |
