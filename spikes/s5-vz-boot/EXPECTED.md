# S5 vz-boot — Expected Assertions and Parity-Audit Mapping

**Honesty statement:** This file describes what each assertion in `run-s5.sh`
proves and which parity-audit row it closes. None of these assertions have
been executed yet. Until `run-s5.sh` runs green on a real ARM Mac (EC2
...
|-----------|---------|-----------------|--------|--------|
| A1 | `lightr run --engine vz @img/alpine -- /bin/echo s5-boot-ok` | exit 0, stdout has `s5-boot-ok`, exit != 255 | Full vz boot chain is live; vsock channel delivers real exit 0 | F-205 |
