...
pub(crate) fn materialize_from_digest(
    ...
    for entry in &manifest.entries {
        match entry {
            ...
            lightr_core::Entry::File { path, mode, digest, .. } => {
            ...
            Instr::Run { argv } => {
                ...
                if code != 0 {
                    return Err(LightrError::InvalidManifest(format!(
                        "RUN step exited with code {code}: {:?}", argv
                    )));
                }
            }
            Instr::Copy { src, dest } => {
...
pub fn step_reads_clock_or_net(argv: &[String]) -> bool {
    let cmd = argv.join(" ");
    let patterns = [
        "date", "curl", "wget", "fetch", "apt-get", "apk", "yum", "pip", "npm", "cargo install",
    ];
...
mod tests {
    ...
    fn step_reads_clock_or_net_heuristic() {
        ...
        let echo_cmd = vec!["/bin/sh".to_string(), "-c".to_string(), "echo hi".to_string()];
    ...
    fn build_memoization_scratch_copy_run() {
        ...
        let report1 = build(ctx.path(), &df_path, "test-build",
            lightr_engine::EngineKind::Native, &store).unwrap();
        assert_eq!(report1.steps, 3);
        assert_eq!(report1.cached_steps, 0);
        let c1: u32 = std::fs::read_to_string(&counter_file).unwrap().trim().parse().unwrap();
        assert_eq!(c1, 1, "RUN should increment to 1");
        let report2 = build(ctx.path(), &df_path, "test-build",
            lightr_engine::EngineKind::Native, &store).unwrap();
        assert_eq!(report2.cached_steps, 3, "all steps must be cache hits");
        let c2: u32 = std::fs::read_to_string(&counter_file).unwrap().trim().parse().unwrap();
        assert_eq!(c2, 1, "counter must NOT increment on cache hit");
        std::fs::write(&src_file, b"changed").unwrap();
        let report3 = build(ctx.path(), &df_path, "test-build",
            lightr_engine::EngineKind::Native, &store).unwrap();
        assert!(report3.cached_steps < 3, "COPY+RUN must not be cached after file change");
        let c3: u32 = std::fs::read_to_string(&counter_file).unwrap().trim().parse().unwrap();
    ...
    fn build_hydrate_final_tree() {
        ...
        let df_path = ctx.path().join("Dockerfile");
        std::fs::write(&df_path,
            "FROM scratch\nCOPY src.txt /src.txt\nRUN echo built\n").unwrap();
        let store = Store::open(store_tmp.path().join("store")).unwrap();
        let report = build(ctx.path(), &df_path, "test-hydrate",
            lightr_engine::EngineKind::Native, &store).unwrap();
        ...
        assert!(dest.join("src.txt").exists(), "/src.txt must be in hydrated tree");
