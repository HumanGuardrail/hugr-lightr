...
pub(crate) struct RunArgs {
    ...
    /// seccomp=<path>). Value is "default" for the BUILT-IN curated profile (a
    /// default-deny allow-list derived from the Docker default profile), a PATH
    /// to an OCI seccomp JSON profile, or "unconfined" to run explicitly without
    /// a seccomp filter. WP-#108: enforced ONLY on the `ns` engine (NO_NEW_PRIVS +
    /// a compiled classic-BPF filter installed right before exec); native/vz
    /// honest-error (exit 2).
